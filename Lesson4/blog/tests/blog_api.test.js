...
describe('when there are initally some blogs saved', () => {
	...
	describe('addition of a new blog', () => {
		...
		test('valid blogs can be added', async() => {
			...
			let token = (await api.post('/api/login').send({username: 'root', password: 'secret'})).body.token 
	...
	describe('updating a blog', () => {
		test('succeeds with status code 204 if id is valid', async () => { 
			...
			let token = (await api.post('/api/login').send({username: 'root', password: 'secret'})).body.token 
			await api
				.put(`/api/blogs/${blogToUpdate.id}`).set('Authorization', 'Bearer ' + token).send(newBlog)
				.expect(200)
